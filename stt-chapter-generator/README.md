...
```bash
cd chapter-generator
python3 -m pip install -r requirements.txt
```

`ModuleNotFoundError`가 나오면 현재 터미널의 `python3`에 의존성이 설치되지 않은 상태입니다. 같은 폴더에서 설치 명령을 다시 실행한 뒤 진행합니다.
...
```bash
python3 chapterize.py data/transcripts/audio.transcript.json
```
